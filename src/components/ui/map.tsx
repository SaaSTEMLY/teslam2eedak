...
function MapLayers({
  ...
  // Set initial selected tile layer (render-time state adjustment)
...
function MapDrawControl({
  ...
  const featureGroupCallbackRef = useCallback((node: L.FeatureGroup | null) => {
  ...
  return (
    ...
    >
      <LeafletFeatureGroup ref={featureGroupCallbackRef} />
      <MapControlContainer className={cn(position, className)}>
...
function MapDrawEdit({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/immutability -- configuring leaflet-draw global locale
...
function useDebounceLoadingState(delay = 200) {
  ...
  // Immediately hide loading when isLoading becomes false (render-time state adjustment)
